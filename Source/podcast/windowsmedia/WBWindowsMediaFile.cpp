                ...
                , int pixelWidth, int pixelHeight, int bitsPerPixel)
{
    ...
    if (FAILED(mWMProfileManager->LoadProfileByData(reinterpret_cast<const WCHAR *>(profileData.utf16()), &mWMProfile)))
    {
    ...
    if (FAILED(mWMWriter->SetOutputFilename(reinterpret_cast<const WCHAR *> (videoFileName.utf16()))))
    {
...
bool WBWindowsMediaFile::close()
{
    ...
    if(mChapters.length() > 0)
    {
        ...
        editor->Open(reinterpret_cast<const WCHAR *> (mVideoFileName.utf16()));
