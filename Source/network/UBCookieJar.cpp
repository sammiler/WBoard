...
UBCookieJar::UBCookieJar(QObject *parent)
    ...
    , mLoaded(false)
    , mSaveTimer(new WBAutoSaver(this))
...
void UBCookieJar::load()
{
    ...
    QSettings cookieSettings(WBSettings::userDataDirectory() + QLatin1String("/cookies.ini"), QSettings::IniFormat);
...
QList<QNetworkCookie> UBCookieJar::cookiesForUrl(const QUrl &url) const
{
    ...
    QWebEngineProfile *profile = QWebEngineProfile::defaultProfile(); // 获取默认配置文件
